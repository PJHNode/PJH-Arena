...
(function () {
  ...
  async function renderMiningGraph() {
    try {
      ...
      ctx.strokeStyle = "#f5c869"; ctx.lineWidth = 2; ctx.beginPath();
      points.forEach((v, i) => {
      ...
      points.forEach((v, i) => {
        const x = (i / Math.max(1, points.length - 1)) * (w - 10) + 5;
        const y = h - 5 - (v / max) * (h - 15);
        ctx.fillStyle = "#f5c869"; ctx.beginPath(); ctx.arc(x, y, 2.5, 0, Math.PI * 2); ctx.fill();
      });
    } catch (e) {}
  }
  ...
  function raidBossSvgHtml(bossName, hpPct) {
    ...
    return (
      ...
      '<stop offset="100%" stop-color="#060809" stop-opacity="0"/>' +
      "</radialGradient></defs>" +
      ...
      '<polygon points="100,45 145,68 145,132 100,155 55,132 55,68" fill="#10151c" stroke="' + theme.c1 + '" stroke-width="2"/>' +
